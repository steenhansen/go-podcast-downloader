...
func setUp() models.ProgBounds {
	...
	test_helpers.DirRemove(progPath + "/final-media-name-m/")
...
const expectedConsole string = `
 Adding 'final-media-name-m'
        Downloading 'final-media-name-m' podcast, 3 files, hit 's' to stop
...
Into 'final-media-name-m'  
`
const expectedBads = ""

func Test_5_FinalMediaName(t *testing.T) {
	progBounds := setUp()
	cleanArgs := []string{"file-name.go", "http://rss.FinalMediaName/podcast.xml", "final-media-name-m"}
	...
	podDir := progBounds.ProgPath + "/final-media-name-m"
	if _, err = os.Stat(podDir); err != nil {
		t.Fatal(podDir + " directory does not exist")
	}

	file1 := progBounds.ProgPath + "/final-media-name-m/final-a.txt"
	if _, err = os.Stat(file1); err != nil {
		t.Fatal(err)
	}

	file2 := progBounds.ProgPath + "/final-media-name-m/final-b.txt"
	if _, err = os.Stat(file2); err != nil {
		t.Fatal(err)
	}

	file3 := progBounds.ProgPath + "/final-media-name-m/final-c.txt"
