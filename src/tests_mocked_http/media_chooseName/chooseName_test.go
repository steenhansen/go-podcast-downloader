...
func setUp() models.ProgBounds {
	...
	test_helpers.DirRemove(progPath + "/choose Name/")
...
const expectedConsole string = `
 Adding 'choose Name'
        Downloading 'choose Name' podcast, 3 files, hit 's' to stop
...
func Test_1_ByNameOrUrl_AddByUrlAndName(t *testing.T) {
	...
	podDir := progBounds.ProgPath + "/choose Name"
	if _, err = os.Stat(podDir); err != nil {
		t.Fatal(podDir + " directory does not exist")
	}

	file1 := progBounds.ProgPath + "/choose Name/first-title.txt"
	if _, err = os.Stat(file1); err != nil {
		t.Fatal(err)
	}

	file2 := progBounds.ProgPath + "/choose Name/second-title.txt"
	if _, err = os.Stat(file2); err != nil {
		t.Fatal(err)
	}

	file3 := progBounds.ProgPath + "/choose Name/third-title.txt"
