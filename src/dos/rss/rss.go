...
func DownloadAndWriteFile(ctx context.Context, mediaUrl, filePath string, minDiskMbs int, httpMedia models.HttpFn) (int, error) {
	respMedia, err := httpMedia(ctx, mediaUrl, consts.RETRIES_MEDIA_FILES)
	if err != nil {
...
func retryHttp(ctx context.Context, numRetries int, tryHttpMedia func() (*http.Response, error)) (respMedia *http.Response, err error) {
	...
	if err != nil {
		retryNotHttpMess := retryCount + ", for " + err.Error()
		return badRetryHttp(retryNotHttpMess, flaws.FLAW_E_40, err)
	}
...
func HttpReal(ctx context.Context, mediaUrl string, numRetries int) (*http.Response, error) {
	respMedia, err := retryHttp(ctx, numRetries, func() (*http.Response, error) { return callHttpMedia(ctx, mediaUrl) })
	if ctx.Err() == context.Canceled {
		return nil, context.Canceled
	}
	if err != nil {
